...
export const api = {
  ...
  aiRiskAssess: (filingId: string) =>
    ...
    }>(`/ai/risk-assess/${filingId}`, { method: 'POST' }),
  ...
  aiQuery: (question: string) =>
    request<{
      answer: string;
      relatedFilings: Array<{ id: string; title: string; filingNumber: string }>;
    }>('/ai/query', {
      method: 'POST',
