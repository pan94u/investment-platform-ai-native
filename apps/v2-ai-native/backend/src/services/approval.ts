...
import type { RiskAssessment, ApprovalContextSnapshot } from '@filing/shared';
import { db } from '../lib/db.js';
import { generateId } from '../lib/id.js';

/** 审批上下文快照存储（PoC 内存存储） */
const contextSnapshots = new Map<string, ApprovalContextSnapshot>();
...
/** 创建审批上下文快照（记录审批人看到了什么） */
export function createContextSnapshot(
  approvalId: string,
  filingId: string,
  summaryShown: string,
  riskPanelShown: RiskAssessment,
  ...
  contextSnapshots.set(approvalId, snapshot);
  return snapshot;
}

/** 获取审批上下文快照 */
export function getContextSnapshot(approvalId: string): ApprovalContextSnapshot | undefined {
  return contextSnapshots.get(approvalId);
}
