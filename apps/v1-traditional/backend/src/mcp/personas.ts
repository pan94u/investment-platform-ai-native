...
// ---------------------------------------------------------------------------
// Persona Definitions — 扩展支持 V1 审批能力
// ---------------------------------------------------------------------------
...
- 透明可解释：每个建议都说明依据`,
  ...
  'filing-approver': {
    ...
    allowedTools: [
      ...
      'filing_stats', 'filing_anomaly_detect', 'filing_extract_from_doc',
...
- 公正客观：不对审批结果做倾向性建议`,
  ...
  'filing-strategist': {
    ...
    name: '投资策略分析师',
    description: '高级分析角色，可访问全量数据进行战略分析和系统管理',
    allowedTools: [
      'filing_get', 'filing_list', 'filing_history', 'filing_risk_assess',
      'filing_stats', 'filing_anomaly_detect', 'filing_extract_from_doc',
      'filing_create', 'filing_update', 'filing_submit', 'filing_recall',
...
export function isToolAllowed(personaId: string, tool: string): boolean {
  ...
  return persona ? persona.allowedTools.includes(tool) : false;
}

export function getSystemPrompt(personaId: string): string {
  return personas[personaId]?.systemPrompt ?? '你是投资管理平台的 AI 助手。';
}
