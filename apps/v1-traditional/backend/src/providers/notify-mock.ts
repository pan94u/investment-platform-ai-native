...
export class MockNotifyProvider implements NotifyProvider {
  async pushTodo(payload: TodoPayload): Promise<string | null> {
    const groupLabel = payload.groupName ? `[${payload.groupName}]` : '';
    console.log(`[MockNotify] pushTodo → ${payload.approverName}: "${payload.filingTitle}" (${payload.stage}${groupLabel} L${payload.level})`);
    return null;
  }
